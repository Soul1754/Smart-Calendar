...
class ChatbotService {
  ...
  async processMessage(userId, message, options = {}) {
    try {
      ...
      if (/^cancel$/i.test(message.trim())) {
        ...
        return { success: true, message: "Meeting scheduling cancelled.", followUp: false };
      }

      // If awaiting slot choice (alternative suggestions after conflict)
      if (existingSession && existingSession.intentType === 'create_meeting' && existingSession.stage === 'awaiting_slot_choice') {
        const choice = this.parseSlotChoice(message, existingSession.availableSlots || []);
        if (!choice) {
          return { success: true, followUp: true, message: 'Please choose a slot by number (e.g. 1) or provide a time like 14:30.', availableSlots: existingSession.availableSlots };
        }
  ...
  const finalize = await this.finalizeMeetingCreation(user, existingSession, userId, { timezone: options.timezone });
        return finalize;
      }
  ...
  const intent = await this.analyzeIntent(message, options.model);
      ...
      if (existingSession && existingSession.intentType === 'create_meeting' && (intent.type !== 'check_schedule' || existingSession.expected)) {
        ...
        if (heuristic.date && !intent.params?.date) intent.params = { ...(intent.params||{}), date: heuristic.date };
        if (heuristic.time && !intent.params?.time) intent.params = { ...(intent.params||{}), time: heuristic.time };
        // Interpret raw answer for expected field
        if (existingSession.expected) {
          const interpreted = this.interpretField(existingSession.expected, message);
          if (interpreted !== undefined && interpreted !== null && interpreted !== '') {
            intent.params = intent.params || {};
            intent.params[existingSession.expected] = interpreted;
          }
        }
        // Merge
        existingSession.params = this.mergeMeetingParams(existingSession.params, intent.params || {});
        existingSession.updatedAt = Date.now();
        const { missing, normalized } = this.normalizeAndValidateMeetingParams(existingSession.params);
        ...
        return { success: true, followUp: true, message: promptMap[nextParam] || `Please provide ${nextParam}.`, pending: missing, collectedParams: normalized };
      }
      ...
      if (intent.type === 'create_meeting') {
        ...
        const { missing, normalized } = this.normalizeAndValidateMeetingParams(initialParams);
        if (missing.length === 0) {
          // All provided – finalize (conflict check)
          const tempSession = { intentType: 'create_meeting', params: normalized };
          const result = await this.finalizeMeetingCreation(user, tempSession, userId, { transient: true, timezone: options.timezone });
          return result;
        }
  ...
  async createMeeting(user, params, options = {}) {
    try {
      ...
      const tz = options.timezone || params.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC';
      ...
      const eh = Math.floor(endMinutesInDay / 60).toString().padStart(2,'0');
  ...
  normalizeAndValidateMeetingParams(params) {
    ...
    if (normalized.duration) {
      ...
      if (!isNaN(dur) && dur > 0 && dur < 24*60) normalized.duration = dur; else delete normalized.duration;
    }
    const required = this.requiredMeetingFields;
    const missing = required.filter(r => !normalized[r] || (r==='attendees' && (!normalized.attendees || normalized.attendees.length===0)));
  ...
  heuristicExtractDateTime(text) {
    ...
    if (/^today$/.test(lower)) out.date = now.toISOString().split('T')[0];
      ...
      const d = new Date(now.getTime() + 86400000); out.date = d.toISOString().split('T')[0];
    } else {
    ...
    if (!out.date) {
      ...
      if (dmy) {
        ...
        if (parseInt(day)<=31 && parseInt(month)<=12) out.date = `${yyyy}-${month}-${day}`;
      }
    }
    if (!out.date) {
    ...
    const timeMatch = text.match(/\b(\d{1,2})(:(\d{2}))?\s*(am|pm)\b/i) || text.match(/\b([01]?\d|2[0-3]):[0-5]\d\b/);
    if (timeMatch) {
  ...
  interpretField(field, raw) {
    ...
    switch(field) {
      ...
      case 'date': { const h = this.heuristicExtractDateTime(value); return h.date || value; }
      case 'time': { const h = this.heuristicExtractDateTime(value); return h.time || value; }
      case 'attendees': return this.extractEmails(value);
  ...
  const start = new Date(startLocalString);
    if (isNaN(start.getTime())) {
      ...
      return { success: true, followUp: true, message: 'I could not parse that date/time. Please provide date (YYYY-MM-DD).', pending: ['date','time'] };
    }
    const duration = params.duration || 30;
    const end = new Date(start.getTime() + duration*60000);
    const conflicts = await this.detectConflicts(user, start, end);
    if (conflicts.length) {
      const alternatives = this.generateAlternativeSlots(conflicts, start, duration);
      if (alternatives.length) {
        session.stage = 'awaiting_slot_choice';
        session.availableSlots = alternatives.map((slot, idx) => ({ index: idx+1, start: slot.start, end: slot.end, label: this.friendlySlotLabel(slot.start, slot.end), score: slot.score }));
        return { success: true, followUp: true, message: `That time conflicts. Choose a slot: ${session.availableSlots.map(s=>`${s.index}) ${s.label}`).join('  ')}`, availableSlots: session.availableSlots };
      }
      return { success: false, message: 'Requested time conflicts and no alternatives found (business hours 09:00-17:00).' };
    }
  const creationResult = await this.createMeeting(user, params, { timezone: options.timezone });
  ...
  async detectConflicts(user, start, end) {
    try {
      let events = [];
      if (user.googleAccessToken) {
        events = await this.calendarService.getGoogleEvents(user, { timeMin: new Date(start.getFullYear(), start.getMonth(), start.getDate()).toISOString(), timeMax: new Date(start.getFullYear(), start.getMonth(), start.getDate(),23,59,59).toISOString() });
      } else if (user.microsoftAccessToken) {
        events = await this.calendarService.getMicrosoftEvents(user);
      }
      ...
      console.error('Conflict detection failed', e); return [];
    }
  }
  generateAlternativeSlots(conflicts, desiredStart, duration) {
    const dayStart = new Date(desiredStart); dayStart.setHours(9,0,0,0);
    const dayEnd = new Date(desiredStart); dayEnd.setHours(17,0,0,0);
    const busy = conflicts.map(ev => ({ start: new Date(ev.start?.dateTime || ev.start?.date), end: new Date(ev.end?.dateTime || ev.end?.date) }));
    busy.push({ start: desiredStart, end: new Date(desiredStart.getTime()+duration*60000) });
    busy.sort((a,b)=>a.start-b.start);
    const merged=[]; for(const slot of busy){ if(!merged.length){merged.push(slot);continue;} const last=merged[merged.length-1]; if(slot.start<=last.end){ if(slot.end>last.end) last.end=slot.end;} else merged.push(slot);} 
    const free=[]; let cursor=dayStart; for(const b of merged){ if(b.end<=dayStart||b.start>=dayEnd) continue; if(b.start>cursor) free.push({start:new Date(cursor),end:new Date(Math.min(b.start,dayEnd))}); if(b.end>cursor) cursor=new Date(Math.max(b.end,cursor)); if(cursor>=dayEnd) break;} if(cursor<dayEnd) free.push({start:new Date(cursor),end:new Date(dayEnd)});
    const candidates=[]; const step=30*60000; for(const f of free){ let s=new Date(f.start); while(s.getTime()+duration*60000<=f.end.getTime()){ const e=new Date(s.getTime()+duration*60000); candidates.push({start:new Date(s), end:e}); s=new Date(s.getTime()+step);} }
    const midday=13; return candidates.map(c=>{ const diff=Math.abs(c.start-desiredStart)/3600000; const midDiff=Math.abs(c.start.getHours()+c.start.getMinutes()/60 - midday); const score=1/(1+diff)*0.6 + (1-Math.min(midDiff/8,1))*0.4; return {...c, score:Number(score.toFixed(3))}; }).sort((a,b)=>b.score-a.score).slice(0,5);
  }
  parseSlotChoice(message, slots){ if(!slots.length) return null; const num=message.trim().match(/^([1-9])$/); if(num){ const idx=parseInt(num[1],10)-1; return slots[idx]; } const h=this.heuristicExtractDateTime(message); if(h.time){ return slots.find(s=>s.start.toISOString().substring(11,16)===h.time) || null; } return null; }
  friendlySlotLabel(start,end){ return `${start.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}-${end.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}`; }
}
