...
router.post("/unified/findAvailableSlots", auth, async (req, res) => {
  try {
    ...
    const normAttendees = attendees.map(a => {
      if (typeof a === "string") return { email: a.trim(), provider: detectProvider(a.trim()) };
      const email = a.email?.trim();
      const prov = (!a.provider || a.provider === 'auto') ? detectProvider(email) : a.provider;
    ...
    const bizStart = new Date(targetDate); bizStart.setHours(businessHours.start ?? 9,0,0,0);
    const bizEnd = new Date(targetDate); bizEnd.setHours(businessHours.end ?? 17,0,0,0);
    if (bizEnd <= bizStart) return res.status(400).json({ message: "businessHours invalid" });
  ...
  const googleEmails = normAttendees.filter(a => a.provider === "google").map(a => a.email);
      const items = [{ id: "primary" }, ...googleEmails.map(e => ({ id: e }))];
      try {
    ...
    if (calId !== 'primary') unreachable.push({ email: calId, attempted: ['google'], reason: 'Google freeBusy error' });
            continue;
          }
            (data.busy || []).forEach(b => busySlots.push({ start: b.start, end: b.end }));
        }
      } catch (e) {
        console.error("Google freeBusy error (gracefully continuing):", e?.response?.data || e?.message || e);
        ...
        googleEmails.forEach(email => {
          ...
          unreachable.push({ email, attempted: ['google'], reason: 'freeBusy request failed' });
        });
      }
    } else if (normAttendees.some(a => a.provider === "google")) {
      // Organizer lacks Google token but google attendees listed
  normAttendees.filter(a => a.provider === "google").forEach(a => { inaccessible.google.push(a.email); unreachable.push({ email: a.email, attempted: ['google'], reason: 'Organizer missing Google token' }); });
    }

    // MICROSOFT organizer events (cannot read other MS attendees without extra permissions)
    if (user.microsoftAccessToken) {
      try {
        const msEventsResp = await axios.get("https://graph.microsoft.com/v1.0/me/calendarView", {
          params: {
      ...
      const msSchedEmails = normAttendees.filter(a => a.provider === 'microsoft' || a.provider === 'unknown').map(a => a.email);
      if (msSchedEmails.length) {
        try {
          const scheduleResp = await axios.post('https://graph.microsoft.com/v1.0/me/calendar/getSchedule', {
            ...
            if (v.scheduleItems) {
              v.scheduleItems.forEach(item => {
                if (item.start?.dateTime && item.end?.dateTime) {
                  busySlots.push({ start: item.start.dateTime, end: item.end.dateTime });
                }
              });
            } else {
              unreachable.push({ email: v.scheduleId, attempted: ['microsoft'], reason: 'No schedule items returned' });
            }
          });
          // Mark those without any scheduleItems as inaccessible
          msSchedEmails.forEach(email => {
            if (!value.find(v => v.scheduleId === email && v.scheduleItems && v.scheduleItems.length)) {
              inaccessible.microsoft.push(email);
            }
          });
        } catch (e) {
          console.error('Microsoft getSchedule error:', e?.response?.data || e?.message || e);
          msSchedEmails.forEach(email => { inaccessible.microsoft.push(email); unreachable.push({ email, attempted: ['microsoft'], reason: 'getSchedule failed' }); });
        }
      }
    } else if (normAttendees.some(a => a.provider === "microsoft")) {
      normAttendees.filter(a => a.provider === "microsoft").forEach(a => { inaccessible.microsoft.push(a.email); unreachable.push({ email: a.email, attempted: ['microsoft'], reason: 'Organizer missing Microsoft token' }); });
    }

    if (!user.googleAccessToken && !user.microsoftAccessToken) {
      return res.status(400).json({ message: "No connected calendar (Google or Microsoft) for organizer" });
    }

    // Normalize and merge busy slots
    const normalizedBusy = busySlots.map(b => ({
      start: new Date(b.start),
      end: new Date(b.end)
    })).filter(b => !isNaN(b.start) && !isNaN(b.end) && b.end > b.start);

    const mergedBusy = mergeBusy(normalizedBusy);
    const availableRaw = computeAvailable(bizStart, bizEnd, mergedBusy, durationMs, incrementMinutes * 60000);

    // Score: prefer slots centered near 13:00 (1 PM)
    const scored = availableRaw.map(slot => {
      const midpoint = new Date((slot.start.getTime() + slot.end.getTime()) / 2);
    ...
    }).sort((a,b) => b.score - a.score || new Date(a.startTimeISO) - new Date(b.startTimeISO))
      .slice(0, maxResults);
    ...
    res.json({
      ...
      unavailableAttendees: {
        ...
        microsoft: inaccessible.microsoft.length ? inaccessible.microsoft : undefined
      },
...
function detectProvider(email) {
  ...
  if (/@(outlook\.com|hotmail\.com|live\.com|office365\.com|microsoft\.com)$/.test(lower)) return "microsoft";
  return 'unknown';
}
...
function computeAvailable(windowStart, windowEnd, busyMerged, durationMs, stepMs) {
  const freeBlocks = [];
  let cursor = new Date(windowStart);
  for (const b of busyMerged) {
    if (b.end <= windowStart || b.start >= windowEnd) continue;
    if (b.start > cursor) {
      freeBlocks.push({ start: new Date(cursor), end: new Date(Math.min(b.start.getTime(), windowEnd.getTime())) });
    }
    if (b.end > cursor) cursor = new Date(Math.max(b.end.getTime(), cursor.getTime()));
    if (cursor >= windowEnd) break;
  }
  if (cursor < windowEnd) freeBlocks.push({ start: new Date(cursor), end: new Date(windowEnd) });
